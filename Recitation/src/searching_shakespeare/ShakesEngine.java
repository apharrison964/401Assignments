...
public class ShakesEngine {
	...
	public static ArrayList<String> preprocessLines(ArrayList<String> lines) {
		...
		for (String line : lines) {
			...
			processed_lines
					.add(line.replaceAll("\\p{Punct}", "").toLowerCase());
		}
		return processed_lines;
	}

	// Build an index structrure on given lines. The index structure is a map
	// from
	...
	// elements are the line numbers where the key occurs. At the end, the size
	// of
	// each ArrayList should equal to the number of occurences of the key.
	public static HashMap<String, ArrayList<Integer>> buildIndex(
			ArrayList<String> lines) {
		// **** TODO ****
		// Create a hashmap whose key type is String and Value type is ArrayList
		// of integer.
		...
		for (String line : lines) {
			...
			// **** TODO ****
			// Index each word by adding the line index to the ArrayList
			// associated
			// to the word. The first time we see a word, we create an ArrayList
			// for
			// it.
			while (scanner.hasNext()) {
				...
				// **** TODO ****
				// Create a new ArrayList if this is the first time we see this
				// word.
				// You can use containsKey() method to check if a key exists.

				// **** TODO ****
				// Add current line number to the ArrayList associated to this
				// word.
	...
	public static void main(String[] args) throws IOException {
		...
		if (index.containsKey(query)) {
			...
			// **** TODO ****
			// Get the Arraylist corresponding to the query from the hashmap and
			// name
			// it "retreived_lines"
			...
			// **** TODO ****
			// Print out each line which has the query word. Remember we have
			// the
