...
/*
 * Code inside class is like a blueprint, class itself may have a method
 * Static are fields and methods that are associated with the class of the whole
 * Objects are states, if it has certain fields the values are the state
 * Design of an object should reflect its purpose
 * Do not say one real value == another real value
 * Use epsilon bounds, you have to decide how close it close enough
 * Take two real values tring to compare, subtract, take abs value, and compare (less than)
 * to epsilon value
 */
